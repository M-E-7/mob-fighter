...
const _COLOR_ACCENT := Color(0.2, 1.0, 0.35)
...
@onready var _sectors_label: Label = $Dim/CenterContainer/VBox/StatsPanel/StatsVBox/SectorsLabel
@onready var _new_run_button: Button = $Dim/CenterContainer/VBox/NewRunButton
...
func setup(player_count: int) -> void:
	...
	_time_label.text = "Time: %d:%02d" % [int(t) / 60, int(t) % 60]
	...
	_sectors_label.text = "Sectors cleared: %d" % RunState.current_level
	_new_run_button.pressed.connect(func() -> void:
		RunState.reset()
...
func _apply_theme() -> void:
	...
	_title.modulate = Color(0.3, 1.7, 1.4)

	var panel_style := StyleBoxFlat.new()
	panel_style.bg_color = _COLOR_BG
	panel_style.border_color = Color(0.2, 0.5, 0.35)
	...
	for lbl in [$Dim/CenterContainer/VBox/StatsPanel/StatsVBox/TimeLabel,
			...
			$Dim/CenterContainer/VBox/StatsPanel/StatsVBox/SectorsLabel]:
		(lbl as Label).add_theme_font_size_override("font_size", 22)
		(lbl as Label).modulate = Color(0.9, 0.9, 1.0)

	_style_button(_new_run_button, _COLOR_ACCENT, Color(0.0, 0.15, 0.08))
