...
- Do not set `Camera2D.ignore_rotation = false` when you also need a fixed (axis-aligned) camera mode. Toggle it per-frame based on the active mode.
- Do not make `_camera_p1` (or any camera with a world-space position offset) a child of a rotating node. Setting `global_position` stores a local offset that the parent's rotation will silently shift every physics frame until `_process` corrects it, causing jitter. Keep cameras as direct children of their subviewport and sync position manually.
