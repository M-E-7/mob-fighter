...
Global autoloads: `EventBus` (`Autoloads/event_bus.gd`), `GameConfig` (`Autoloads/game_config.gd`), `MusicManager` (`Autoloads/music_manager.gd`), `AdvancedConfig` (`Autoloads/advanced_config.gd`). *Planned (roguelike roadmap, see `docs/claude-roguelike.md`):* `RunState` (`Autoloads/run_state.gd`), `MetaProgression` (`Autoloads/meta_progression.gd`)  
...
- **Roguelike run loop (planned — see `docs/claude-roguelike.md`):** a run is a sequence of **sectors** (`basic_level.tscn` reused, parameterized by `RunState.current_level`) — collect **Bits** (currency) → meet the sector objective → an **ExitPort** spawns → the **Sandbox** shop → next sector → final rogue-AI boss → win. State splits across three tiers: `GameConfig` (settings + results), `RunState` (one run, wiped on `reset()`), `MetaProgression` (persistent `user://` unlocks). The in-run level-up is removed — `XPComponent` banks orbs into `RunState.currency`. Modifiers ("**Daemons**") are event-hook passives; both new autoloads are no-`class_name`.
