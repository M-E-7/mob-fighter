...
Cameras are created at runtime in `basic_level.gd` — there is no camera component. `_camera_p1` is added as a child of `Player` (auto-follows position); `_camera_p2` is added to `SubViewportP2` and position-synced manually in `_process`.

Two modes controlled by `GameConfig.camera_relative_mode`:

**Fixed mode** (default off): `Camera2D.ignore_rotation = true` (Godot default). Camera inherits the player node's transform but ignores rotation, so the view stays axis-aligned regardless of ship heading.

**Relative mode**: Camera instantly rotates to keep the mouse direction pointing toward screen-top, while the ship turns toward it at its normal `turn_speed`.
...
- Each `_process` frame, `_display_cam_angle` is lerped toward `_rel_cam_angle` using `GameConfig.camera_smoothing` as the lerp speed: `lerp_angle(_display_cam_angle, _rel_cam_angle, clamp(camera_smoothing * delta, 0, 1))`. The camera's `global_rotation` is set to `_display_cam_angle`.
- `Camera2D.ignore_rotation = false` is required for the rotation to affect the viewport. Set each frame in relative mode; reverted to `true` in fixed mode to avoid one-frame jitter from the player's inherited rotation.
...
### Key gotchas
- `Camera2D.ignore_rotation` defaults to `true` — setting `camera.rotation` or `camera.global_rotation` has **no visual effect** until this is set to `false`.
- Use `global_rotation` (not `rotation`) when the camera is a child of a rotating node; `rotation` is local and the parent's rotation accumulates on top.
- Do not set `ignore_rotation = false` permanently when also using fixed mode — the camera inherits the player's rotation between `_physics_process` (where the player rotates) and `_process` (where the camera is corrected), causing one-frame jitter.
