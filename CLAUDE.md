...
- **Roguelike run loop (P1 built; P2–P10 planned — see `docs/claude-roguelike.md`):** a run is a sequence of **sectors** (`basic_level.tscn` reused, parameterized by `RunState.current_level`) — collect **Bits** (currency) → meet the sector objective → an **ExitPort** spawns → the **Sandbox** shop → next sector → final rogue-AI boss → win. State splits across three tiers: `GameConfig` (settings + results), `RunState` (one run, wiped on `reset()`), `MetaProgression` (persistent `user://` unlocks). **Built (P1):** `RunState` is live and is the *sole* subscriber to `EventBus.xp_collected` — it banks each orb into `RunState.currency` and emits `EventBus.currency_changed(total)`; the in-run level-up is gone and `XPComponent` is now a pure stat-upgrade holder. **Planned:** sectors/objectives, `ExitPort`, the **Sandbox** shop, **Daemons** (event-hook passive modifiers), bosses, and `MetaProgression`. The new autoloads are no-`class_name`.
